...
void iplc_sim_process_pipeline_rtype(char *instruction, int dest_reg, int reg1,
...
unsigned int instruction_count =
    0; // home many real instructions ran thru the pipeline
...
typedef struct jump { char instruction[16]; } jump_t;
...
void iplc_sim_finalize() {
  ...
  while (pipeline[FETCH].itype != NOP || pipeline[DECODE].itype != NOP ||
         pipeline[ALU].itype != NOP || pipeline[MEM].itype != NOP ||
...
void iplc_sim_dump_pipeline() {
  ...
  for (i = 0; i < MAX_STAGES; i++) {
    switch (i) {
    case FETCH:
      printf("(cyc: %u) FETCH:\t %d: 0x%x \t", pipeline_cycles,
             pipeline[i].itype, pipeline[i].instruction_address);
      break;
    case DECODE:
      printf("DECODE:\t %d: 0x%x \t", pipeline[i].itype,
             pipeline[i].instruction_address);
      break;
    case ALU:
      printf("ALU:\t %d: 0x%x \t", pipeline[i].itype,
             pipeline[i].instruction_address);
      break;
    case MEM:
      printf("MEM:\t %d: 0x%x \t", pipeline[i].itype,
             pipeline[i].instruction_address);
      break;
    case WRITEBACK:
      printf("WB:\t %d: 0x%x \n", pipeline[i].itype,
  ...
  if (pipeline[WRITEBACK].instruction_address) {
    ...
    if (debug)
      printf("DEBUG: Retired Instruction at 0x%x, Type %d, at Time %u \n",
             pipeline[WRITEBACK].instruction_address, pipeline[WRITEBACK].itype,
             pipeline_cycles);
  }
...
void iplc_sim_parse_instruction(char *buffer) {
  ...
  if (!instruction_hit) {
    // need to subtract 1, since the stage is pushed once more for actual
    // instruction processing
    // also need to allow for a branch miss prediction during the fetch cache
    // miss time -- by
    // counting cycles this allows for these cycles to overlap and not doubly
    // count.

    printf("INST MISS:\t Address 0x%x \n", instruction_address);

    for (i = pipeline_cycles, j = pipeline_cycles; i < j + CACHE_MISS_DELAY - 1;
         i++)
      iplc_sim_push_pipeline_stage();
  } else
  ...
  if (strncmp(instruction, "add", 3) == 0 ||
    ...
    if (sscanf(buffer, "%x %s %s %s %s", &instruction_address, instruction,
               str_dest_reg, str_src_reg, str_src_reg2) != 5) {
    ...
    if (sscanf(buffer, "%x %s %s %s", &instruction_address, instruction,
               str_dest_reg, str_constant) != 4) {
      printf("Malformed RTYPE instruction (%s) at address 0x%x \n", instruction,
             instruction_address);
      exit(-1);
    }
    ...
    if (sscanf(buffer, "%x %s %s %s %x", &instruction_address, instruction,
               reg1, offsetwithreg, &data_address) != 5) {
